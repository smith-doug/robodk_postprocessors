...
class RobotPost(BasePost):
    ...
    # multiple "pages (files)". This can be overriden by RoboDK settings.
    DONT_USE_MFRAME = True  # Set to false to use MFRAME for setting reference frames
    ...
    USE_RELATIVE_JOB = True  # Set to False to always use pulses (Otherwise, it might require a
    # special/paid option
    ...
    # Pulses per degree (provide these in the robot parameters menu: Double click the motoman
    # robot in RoboDK, select "Parameters"
    ...
    PROG_NAME_CURRENT = 'unknown'  # Auto generated name (different from PROG_NAME if we have
    # more than 1 page per program. Example: ProgA2)
    ...
    nAxes = 6  # Important: This is usually provided by RoboDK automatically. Otherwise,
    # override the __init__ procedure.
    AXES_TYPE = ['R', 'R', 'R', 'R', 'R',
                 'R']  # Important: This is usually set up by RoboDK automatically. Otherwise,
    # override the __init__ procedure.
    # 'R' for rotative axis, 'L' for linear axis, 'T' for external linear axis (linear track),
    # 'J' for external rotative axis (turntable)

    # AXES_TYPE = ['R','R','R','R','R','R','T','J','J'] #example of a robot with one external
    # linear track axis and a turntable with 2 rotary axes
    AXES_TRACK = []
    AXES_TURNTABLE = []
    HAS_TRACK = False
    HAS_TURNTABLE = False
        ...
        if new_page:
            ...
            if self.nPages == 0:
                if len(self.PROG_NAMES_MAIN) > 0:
                    print(
                        "Can't split %s: Two or more programs are split into smaller programs" %
                        progname)
                    print(self.PROG_NAMES_MAIN)
                    raise Exception("Only one program at a time can be split into smaller programs")
                self.PROG_NAMES_MAIN.append(
                    self.PROG_NAME)  # add the first program in the list to be genrated as a
                # subprogram call
    ...
    def ProgFinish(self, progname, new_page=False):
        ...
        # if self.nPROGS > 1:
        #    # Motoman does not support defining multiple programs in the same file,
        #    so one program per file
        #    return
        header = ''
        header += '/JOB' + '\n'
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        self.page_size_control()  # Important to control the maximum lines per program and not
        # save last target on new program
    ...
    def MoveL(self, pose, joints, conf_RLF=None):
        ...
        #    if distance(pose.Pos(), self.LAST_POSE.Pos()) < 0.1 and pose_angle_between(pose,
        #    self.LAST_POSE) < 0.1:
        #        return
    ...
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        ...
        if self.LAST_POSE is not None:
            target_id0 = self.add_target_cartesian(self.POSE_FRAME * self.LAST_POSE, joints1,
                                                   conf_RLF_1)
    ...
    def setFrame(self, pose, frame_id, frame_name):
        ...
        if self.DONT_USE_MFRAME:
            ...
            self.RunMessage('%.1f,%.1f,%.1f,%.1f,%.1f,%.1f' % (
                xyzwpr[0], xyzwpr[1], xyzwpr[2], xyzwpr[3], xyzwpr[4], xyzwpr[5]), True)
        else:
            self.POSE_FRAME = eye(4)
            if frame_id is None or frame_id < 0:
                self.RunMessage('Setting Frame %i (%s):' % (self.ACTIVE_FRAME, str(frame_name)),
                                True)
                ...
                self.RunMessage(
                    'Frame %i (%s) should be close to:' % (self.ACTIVE_FRAME, str(frame_name)),
                    True)
                self.RunMessage('%.1f,%.1f,%.1f,%.1f,%.1f,%.1f' % (
                    xyzwpr[0], xyzwpr[1], xyzwpr[2], xyzwpr[3], xyzwpr[4], xyzwpr[5]), True)
    ...
    def setTool(self, pose, tool_id, tool_name):
        ...
        if tool_id is None or tool_id < 0:
            ...
            self.RunMessage('Tool %i (%s) should be close to:' % (self.ACTIVE_TOOL, str(tool_name)),
                            True)
            self.RunMessage('%.1f,%.1f,%.1f,%.1f,%.1f,%.1f' % (
                xyzwpr[0], xyzwpr[1], xyzwpr[2], xyzwpr[3], xyzwpr[4], xyzwpr[5]), True)
    ...
    def setSpeed(self, speed_mms):
        ...
        speedl = max(0.01, min(speed_cm_min,
                               200.0))  # Important! Filter linear speed is in mm/s or cm/min (
...
def test_post():
    ...
    robot.MoveJ(Pose([200, 200, 500, 180, 0, 180]),
